...
class MultipleObjectTracker(object):
    ...
    def update_tracks(self, current_plot):
        ...
        for _plot in plots:
            ...
            if _plot.has_match is False and _plot.plot[0][3]>200 :#avoid the small plot to track
                if const.VERBOSE_LIGHT is True:
        ...
        for t in self.tracks:
            if t.has_match is True:  
                ...
                for _t in self.tracks:
                    ...
                    if  t.label!=_t.label and _t.has_match is False \
                                    and self.indiceFrame>1 and t.is_singular() == False \
                                        and (t.plot[0][3] > 1.5*t.old_plot[0][3] or t.flag_cluster == True or _t.flag_cluster==True):        
                ...
                if n > 1:#>1 because we add at the beginning the matched track   
                    ...
                    for j in range(n):
                        for i,_t_no_assigned in enumerate(liste_track_no_assigned):
                            dt = 1/const.FPS
                            
                            if np.isnan(_t_no_assigned.speed[0][0]) or np.isinf(_t_no_assigned.speed[0][0]) \
                                or np.isnan(_t_no_assigned.speed[0][1]) or np.isinf(_t_no_assigned.speed[0][1]):
                                estime_current_plot = _t_no_assigned.old_plot[0][:2]
                            else:    
                                estime_current_plot = _t_no_assigned.old_plot[0][:2]+_t_no_assigned.speed[0][:2]*dt
...
class Track(object):
    def __init__(self,numPiste):
        ...
        self.flag_touch = 'A'
        self.win = np.zeros((28,28),dtype='float32')
        self.brief = np.zeros((1,200),dtype='float32')
    ...
    def brief_ORB(self, img,kp):
        self.kp, self.descr = self.orb.compute(img, kp)
